...
1. **Grant cluster access permissions** - The IAM policy must also grant permission to access the cluster resources. Use a wildcard (`*`) or follow the instructions for using IAM condition keys with Aurora DSQL.
...
1. **Create a new database role** with the `WITH LOGIN` option:
...
1. **Database Connection**: Use token as password to connect with `admin` role
1. **Database Operations**: Perform administrative tasks, create custom roles

### For Custom Role

1. **Role Creation**: Admin creates custom database role with `CREATE ROLE example WITH LOGIN`
1. **IAM Association**: Admin associates role with IAM identity using `AWS IAM GRANT`
1. **Permission Granting**: Admin grants specific database permissions to custom role
1. **IAM Authentication**: User generates token using IAM identity with `dsql:DbConnect` permission
1. **Database Connection**: User connects using custom role and token
...
1. **Create custom roles**: Issue SQL commands to create new database-level roles
1. **Grant permissions**: Use PostgreSQL GRANT commands to assign specific permissions
1. **Associate with IAM**: Use `AWS IAM GRANT` to link database roles with IAM identities
