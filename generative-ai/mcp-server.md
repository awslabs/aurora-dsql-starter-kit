...
1. You need an AWS account with a role including these permissions:
...
1. Configure AWS credentials with `aws configure` or environment variables
...
- Each MCP server is configured with a [mcp_servers.] table in the config file. Refer to the [Custom Codex Installation Instructions](#codex)
- Warp:
